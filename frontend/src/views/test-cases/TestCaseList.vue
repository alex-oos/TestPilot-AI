  ...
  <div class="flex h-full">
    ...
    <div class="w-[220px] flex-shrink-0 border-r border-gray-200 bg-white overflow-y-auto">
      <div class="p-4">
        ...
        <div
          ...
          :key="mod.name"
          class="group flex items-center justify-between px-3 py-2 rounded-lg cursor-pointer transition-colors mb-1"
          :class="selectedModule === mod.name ? 'bg-indigo-50 text-indigo-700' : 'hover:bg-gray-50 text-gray-700'"
          @click="selectModule(mod.name)"
        >
          <template v-if="renamingModule === mod.name">
            <input
              v-model="renameValue"
              class="flex-1 text-sm px-2 py-0.5 border border-indigo-300 rounded outline-none focus:ring-2 focus:ring-indigo-200"
              @keyup.enter="confirmRenameModule(mod.name)"
              @keyup.escape="renamingModule = null"
              @blur="confirmRenameModule(mod.name)"
              @click.stop
              ref="renameInput"
            />
          </template>
          <template v-else>
            <span class="text-sm truncate flex-1">{{ mod.name }}</span>
            <div class="flex items-center gap-0.5">
              <span class="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full">{{ mod.count }}</span>
              <el-dropdown trigger="click" @click.stop size="small" @command="(cmd: string) => handleModuleAction(cmd, mod.name)">
                <button class="p-0.5 rounded hover:bg-gray-200 opacity-0 group-hover:opacity-100 transition-opacity ml-1" @click.stop>
                  <span class="text-gray-400 text-xs">⋯</span>
                </button>
                <template #dropdown>
                  <el-dropdown-menu>
                    <el-dropdown-item command="rename">重命名</el-dropdown-item>
                    <el-dropdown-item command="delete" divided>删除模块</el-dropdown-item>
                  </el-dropdown-menu>
                </template>
              </el-dropdown>
            </div>
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
      ...
      <div v-else class="flex-1 overflow-hidden p-4">
        <div v-if="!mindMapData || !mindMapData.children?.length" class="h-full flex flex-col items-center justify-center text-gray-400">
          <p class="text-lg mb-2">暂无数据</p>
          <p class="text-sm">请先在侧边栏创建模块，或切换到表格视图新建用例</p>
        </div>
        <MindMapComponent v-else :data="mindMapData" :editable="true" @save="handleMindMapSave" />
      </div>
    </div>
...
const mindMapData = computed<MindMapNode | null>(() => {
  const cases = allCasesForMindMap.value
  const moduleMap = new Map<string, TestCase[]>()
  for (const mod of modules.value) {
    moduleMap.set(mod.name, [])
  }
  for (const tc of cases) {
    const mod = tc.module || '未分配模块'
    if (!moduleMap.has(mod)) moduleMap.set(mod, [])
    moduleMap.get(mod)!.push(tc)
  }
  if (!moduleMap.has('未分配模块')) moduleMap.set('未分配模块', [])
  const children: MindMapNode[] = []
  for (const [modName, modCases] of moduleMap) {
    children.push({
      content: modName,
      children: modCases.map(tc => ({
        content: tc.title,
        children: tc.steps?.map((s, i) => ({
          content: `${i + 1}. ${s.action}`,
          payload: { description: s.expected }
        })) || [],
...
async function fetchAllCasesForMindMap() {
  try {
    const res = await getTestCases({ page: 1, page_size: 500 })
    const payload = res?.data ?? res
    allCasesForMindMap.value = payload?.items ?? payload?.list ?? []
  } catch {
...
function queryModuleSuggestions(queryString: string, cb: (results: any[]) => void) {
  const suggestions = modules.value
    .map(m => ({ value: m.name }))
...
async function fetchList() {
  loading.value = true
  try {
    const res = await getTestCases(query)
    const payload = res?.data ?? res
...
async function fetchModules() {
  try {
    ...
    const serverModules: ModuleInfo[] = rawModules.filter((m: any) => m.name)
    unassignedCount.value = rawModules.find((m: any) => !m.name)?.count ?? 0
...
function confirmAddModule() {
  ...
  modules.value.push({ name, count: 0 })
...
async function confirmRenameModule(oldName: string) {
  ...
  if (mod && mod.count > 0) {
    try {
      const res = await getTestCases({ module: oldName, page: 1, page_size: 500 })
      const payload = res?.data ?? res
...
async function handleMindMapSave(data: MindMapNode) {
  ...
  try {
    let created = 0
    let updated = 0
    for (const modNode of data.children) {
      const moduleName = modNode.content
      if (!modNode.children?.length) continue
      for (const caseNode of modNode.children) {
        const title = caseNode.content
        if (!title?.trim()) continue
        const steps = (caseNode.children || [])
          .map(s => ({ action: s.content?.replace(/^\d+\.\s*/, '') || '', expected: s.payload?.description || '', test_data: '' }))
          .filter(s => s.action.trim())
        const caseData: any = {
          title,
          module: moduleName === '未分配模块' ? null : moduleName,
