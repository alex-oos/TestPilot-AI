...
from app.services.test_case_sync_utils import resolve_case_sync_context
from app.util.module_path_utils import module_prefix_filter
...
async def list_modules(
    ...
    req_stmt = (
        ...
        .where(TestCase.requirement_id.isnot(None), base_filter)
        .group_by(TestCase.requirement_id, Requirement.title, TestCase.module)
        .order_by(Requirement.title, TestCase.module)
    )
    req_rows = (await db.execute(req_stmt)).all()

    groups_map: dict[int, dict] = {}
    for req_id, req_title, module_name, count in req_rows:
        group = groups_map.setdefault(
            int(req_id),
            {
                "requirement_id": int(req_id),
                "requirement_title": req_title or f"需求#{req_id}",
                "count": 0,
                "modules": [],
            },
        )
        ...
        group["count"] += cnt
        if module_name:
            group["modules"].append({"name": module_name, "count": cnt})
        else:
            group.setdefault("_unassigned", 0)
            group["_unassigned"] = group.get("_unassigned", 0) + cnt

    requirement_groups = sorted(groups_map.values(), key=lambda g: g["requirement_title"])
    for group in requirement_groups:
        group["modules"].sort(key=lambda m: m["name"])
        group.pop("_unassigned", None)
    ...
    standalone_modules = [
        ...
        for name, count in (await db.execute(standalone_stmt)).all()
        if name
    ]

    unassigned_stmt = select(func.count(TestCase.id)).where(
        TestCase.module.is_(None),
        base_filter,
    )
    unassigned_count = int((await db.execute(unassigned_stmt)).scalar() or 0)

    flat_modules = [
        *standalone_modules,
        *[
            {"name": mod["name"], "count": mod["count"]}
            for group in requirement_groups
...
async def list_cases(
    ...
    if module:
        if module == "__unassigned__":
            base = base.where(TestCase.module.is_(None))
        else:
            base = base.where(TestCase.module == module)
    elif module_prefix:
        base = base.where(module_prefix_filter(module_prefix))
    if priority:
...
async def batch_adopt(body: dict, db: AsyncSession = Depends(get_db)):
    ...
    for item in body.get("cases") or []:
        ...
        case.title = title
        sync_module, sync_project_id, sync_requirement_id = await resolve_case_sync_context(
            db,
            requirement_id=item.get("requirement_id"),
            project_id=item.get("project_id"),
            ai_module=item.get("module"),
        )
        case.module = sync_module
        ...
        case.project_id = sync_project_id
        case.requirement_id = sync_requirement_id
