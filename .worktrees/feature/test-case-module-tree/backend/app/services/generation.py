...
from app.rag.knowledge_base import ingest_requirement_document, retrieve_kb_context
...
async def process_generation_request(source_type: str, doc_url: Optional[str], file: Optional[UploadFile]) -> Dict[str, Any]:
    ...
    history_context, _ = retrieve_kb_context(
        "generation",
        text_content,
    ...
    routing = resolve_generation_routing(f"{text_content[:2000]}\n{analysis[:2000]}")
