...
from app.services.test_case_sync_utils import coerce_optional_int, resolve_case_sync_context
...
async def _persist_adopted_cases_to_library(
    ...
    *,
    ...
    created = 0
    updated = 0
    resolved_requirement_id = coerce_optional_int(requirement_id)
    resolved_project_id = coerce_optional_int(project_id)
    ...
    for item in cases:
        ...
        if resolved_requirement_id is not None:
            stmt = stmt.where(TestCase.requirement_id == resolved_requirement_id)
...
async def apply_task_decision(
    ...
    *,
    ...
    if normalized == "accepted":
        generation_data = (task.get("phases") or {}).get("generation", {}).get("data") or {}
        cases = generation_data.get("cases") if isinstance(generation_data, dict) else None
        if isinstance(cases, list) and cases and db is not None:
            reviewed_cases = [
            ...
            if reviewed_cases:
                return await update_review_cases(task_id, reviewed_cases, db=db)
