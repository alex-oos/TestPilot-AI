...
4. 飞书写回能力升级：在需求 wiki 下创建“需求名+测试用例”子文档，目标类型为 `mindnote`（思维导图）。
5. 本地文件上传支持图片识别（OCR）。
...
2. 飞书写回不是普通 `docx` 文档：当前目标为 `mindnote` 思维导图。
...
FEISHU_CLI_AS="user"
FEISHU_WIKI_CHILD_OBJ_TYPE="mindnote"
```
...
1. `FEISHU_WIKI_CHILD_OBJ_TYPE="mindnote"`。
2. 使用的是 wiki 链接（不是普通 docx 链接）。
3. 当前飞书账号/应用具备目标文档空间写权限。
